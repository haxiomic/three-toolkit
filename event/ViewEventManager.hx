...
class ViewEventManager {
	...
	static final supportsCoalescedEvents = js.Syntax.code('PointerEvent.prototype.getCoalescedEvents') != null;
	...
	public var listenersAttached(default, null) = false;
	...
	public function removeEventListeners(clearTouchAction = false) {
		if (clearTouchAction) {
			el.style.touchAction = '';
			el.removeAttribute('touch-action');
		}

		el.removeEventListener('gesturestart', cancelEvent, false);
		el.removeEventListener('gesturechange', cancelEvent, false);
	...
	**/
	...
	function onUnifiedPointerDown(e: PointerEvent) {
		updatePointerState(e);
		eventHandler.onPointerDown(e, e.nativeEvent.target == el);
	}
	function onUnifiedPointerMove(e: PointerEvent) {
		updatePointerState(e);
		eventHandler.onPointerMove(e, e.nativeEvent.target == el);
	}
	function onUnifiedPointerUp(e: PointerEvent) {
	...
	function onUnifiedPointerCancel(e: PointerEvent) {
		switch e.pointerType {
			case MOUSE:
				updatePointerState(e);
			case PEN, TOUCH:
	...
	// native pointer
	function onNativePointerDown(e: js.html.PointerEvent) {
		onUnifiedPointerDown(convertPointerEvent(e));
	}
	function onNativePointerMove(e: js.html.PointerEvent) {
		if (supportsCoalescedEvents) {
			for (e in e.getCoalescedEvents()) {
				onUnifiedPointerMove(convertPointerEvent(e));
			}
		} else {
			onUnifiedPointerMove(convertPointerEvent(e));
		}
	}
	function onNativePointerUp(e: js.html.PointerEvent) {
		onUnifiedPointerUp(convertPointerEvent(e));
	}
	function onNativePointerCancel(e: js.html.PointerEvent) {
		onUnifiedPointerCancel(convertPointerEvent(e));
	}
	// native mouse
	function onNativeMouseDown(e: js.html.MouseEvent) {
		executePointerMethodFromMouseEvent(e, onUnifiedPointerDown);
	}
	function onNativeMouseMove(e: js.html.MouseEvent) {
		executePointerMethodFromMouseEvent(e, onUnifiedPointerMove);
	}
	function onNativeMouseForceChange(e: js.html.MouseEvent) {
		executePointerMethodFromMouseEvent(e, onUnifiedPointerMove);
	}
	function onNativeMouseUp(e: js.html.MouseEvent) {
		executePointerMethodFromMouseEvent(e, onUnifiedPointerUp);
	}
	// native touch
	function onNativeTouchStart(e: js.html.TouchEvent) {
		executePointerMethodFromTouchEvent(e, onUnifiedPointerDown);
	}
	function onNativeTouchMove(e: js.html.TouchEvent) {
		executePointerMethodFromTouchEvent(e, onUnifiedPointerMove);
	}
	function onNativeTouchForceChange(e: js.html.TouchEvent) {
		executePointerMethodFromTouchEvent(e, onUnifiedPointerMove);
	}
	function onNativeTouchEnd(e: js.html.TouchEvent) {
		executePointerMethodFromTouchEvent(e, onUnifiedPointerUp);
	}
	function onNativeTouchCancel(e: js.html.TouchEvent) {
		executePointerMethodFromTouchEvent(e, onUnifiedPointerCancel);
	}

	function addPointerEventListeners() {
		// use PointerEvent API if supported
		if (js.Syntax.field(window, 'PointerEvent')) {
			window.addEventListener('pointerdown', onNativePointerDown, useCapture);
			window.addEventListener('pointermove', onNativePointerMove, useCapture);
			window.addEventListener('pointerup', onNativePointerUp, useCapture);
			window.addEventListener('pointercancel', onNativePointerCancel, useCapture);
		} else {
			window.addEventListener('mousedown', onNativeMouseDown, useCapture);
			window.addEventListener('mousemove', onNativeMouseMove, useCapture);
			window.addEventListener('webkitmouseforcechanged', onNativeMouseForceChange, useCapture);
			// window.addEventListener('mouseforcechanged', (e) -> executePointerMethodFromMouseEvent(e, onPointerMove), useCapture);
			window.addEventListener('mouseup', onNativeMouseUp, useCapture);

			window.addEventListener('touchstart', onNativeTouchStart, { capture: useCapture, passive: false }); 
			window.addEventListener('touchmove', onNativeTouchMove, { capture: useCapture, passive: false }); 
			window.addEventListener('touchforcechange', onNativeTouchForceChange, { capture: useCapture, passive: false });
			window.addEventListener('touchend', onNativeTouchEnd, {capture: useCapture, passive: false });
			window.addEventListener('touchcancel', onNativeTouchCancel, { capture: useCapture, passive: false });
		}
	}
	...
	function removePointerEventListeners() {
		// use PointerEvent API if supported
		if (js.Syntax.field(window, 'PointerEvent')) {
			window.removeEventListener('pointerdown', onNativePointerDown, useCapture);
			window.removeEventListener('pointermove', onNativePointerMove, useCapture);
			window.removeEventListener('pointerup', onNativePointerUp, useCapture);
			window.removeEventListener('pointercancel', onNativePointerCancel, useCapture);
		} else {
			window.removeEventListener('mousedown', onNativeMouseDown, useCapture);
			window.removeEventListener('mousemove', onNativeMouseMove, useCapture);
			window.removeEventListener('webkitmouseforcechanged', onNativeMouseForceChange, useCapture);
			// window.removeEventListener('mouseforcechanged', (e) -> executePointerMethodFromMouseEvent(e, onPointerMove), useCapture);
			window.removeEventListener('mouseup', onNativeMouseUp, useCapture);

			window.removeEventListener('touchstart', onNativeTouchStart, useCapture); 
			window.removeEventListener('touchmove', onNativeTouchMove, useCapture); 
			window.removeEventListener('touchforcechange', onNativeTouchForceChange, useCapture);
			window.removeEventListener('touchend', onNativeTouchEnd, useCapture);
			window.removeEventListener('touchcancel', onNativeTouchCancel, useCapture);
		}
	}

	function onNativeWheel(e: js.html.WheelEvent) {
		var bounds = el.getBoundingClientRect();
	...
	function addWheelEventListeners() {
		window.addEventListener('wheel', onNativeWheel, { passive: false, capture: useCapture });
	}

	function removeWheelEventListeners() {
		window.removeEventListener('wheel', onNativeWheel);
	}

	function onNativeKeyDown(e: js.html.KeyboardEvent) {
		var onTargetView = e.target == el;
		eventHandler.onKeyDown(cast e, onTargetView);
	}

	function onNativeKeyUp(e: js.html.KeyboardEvent) {
		var onTargetView = e.target == el;
	...
	function onNativeVisibilityChange() {
		onVisibilityChange();
	}

	function addLifeCycleEventListeners() {
		// life-cycle events
		document.addEventListener('visibilitychange', onNativeVisibilityChange);
	}

	function removeLifeCycleEventListeners() {
		document.removeEventListener('visibilitychange', onNativeVisibilityChange);
	}
